...
public class WorkerThread extends Thread implements Worker {
	...
	public static Worker getWorker() {
		...
		if (worker == null) {
			...
			LoggerAPI.logInfo("Thread is busy so created new thread...");
		}
		return worker;
	}
	...
	private static Worker findFreeWorkerFromThreadResources() {
		for (Object key : THREAD_RESOURCES.keySet()) {
			LoggerAPI.logInfo("Checking thread...");
	...
	public boolean isCurretWorkCompletedByThisWorker() {
		...
		if (threadUtility.get("isCurrentWorkDone") == null) {
			LoggerAPI.logInfo("Curent work done is null..");
			return false;
		}
		LoggerAPI.logInfo("Returning current work done flage..");
	...
	public void run() {
		...
		if (work != null && !isStopCommanExecuted) {
			LoggerAPI.logInfo("Starting work.");
			synchronized (this) {
				work.doWork();
				this.notify();
			}
